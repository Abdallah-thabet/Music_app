...
def albums_list(request):
    ...
    return render(request, 'albums_list.html', {'albums': queryset})


def album_detail(request, id=None):
    queryset = get_object_or_404(Album, id=id)
    return render(request, 'album_details.html', {'instance': queryset})
...
def AlbumCreate(request):
    ...
    return render(request, 'albums_create.html', {'form': form})#? i will try to run the app without it.


def media_error(request):
    return render(request, 'media_error.html')